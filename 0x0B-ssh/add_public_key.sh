#!/usr/bin/env bash
# This script adds the provided SSH public key to the authorized_keys of the ubuntu user on the server

# Public key to be added
PUBLIC_KEY="ssh-rsa "

# Append the public key to the authorized_keys file on the server
ssh -i ~/.ssh/school ubuntu@<server_ip> "echo ${PUBLIC_KEY} >> ~/.ssh/authorized_keys"
